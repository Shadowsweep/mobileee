...
def get_max_smartphones(url, name_class, price_class):
    total_phones = 0
    page_num = 1
    all_data = [["Phone", "Price", "Mobile Link", "Brand"]]  # Header for CSV
    while True:
        ...
        for phone, price, link, brand in zip(phones, prices, links, brands):
            all_data.append([phone, price, link, brand])  # Append data to list
            print(f"Phone: {phone}, Price: {price}, Link: {link}, Brand: {brand}")  # Print for verification
        total_phones += len(phones)
        page_num += 1
    
    # Append data to CSV
    append_to_csv("mobile_data1.csv", all_data)
    return total_phones
